...
from siesta.model.DataModel import Last_Checked_table_schema, EventPair
import logging
...
def _calculate_pairs_stnm(activity_index: Tuple[Trace_ID, Iterable[Event]], last: Optional[Iterable[Tuple[str, str, int]]] | None, lookback: Tuple[int, LookbackType]) -> Tuple[List[Tuple], List[Tuple]]:
    ...
    for key in activity_index_map:
        activity_index_map[key].sort(key=lambda x: x[0])  # sort by timestamp
...
def createTuples(
    ...
    """
    Create event-pair tuples: for each source occurrence, pair it with the
    first target occurrence that comes after it (within the lookback window).

    `i` is a shared lower-bound pointer that advances only when a target
    position is <= the current source position (i.e. the target cannot follow
    this source or any later source).  Crucially, `i` is NOT advanced after a
    successful match, so the same target is reusable by multiple later sources.

    This replaces the old skip-till-next-match (STNM) algorithm whose `prev`
    guard prevented a source from pairing with a target that had already been
    consumed by an earlier source — even when the source legitimately preceded
    that target.  Under STNM a chain join on consecutive pair tables could fail
    because the intermediate activity's target position in pair (A→B) was not
    stored as a source position in pair (B→C).  The new semantics guarantee
    that every position that appears as target_pos in pair (X→Y) also appears
    as source_pos in pair (Y→Z), enabling the adaptive chain join to work
    correctly.

    Returns:
        List of tuples: (key1, key2, trace_id, tsA, tsB, posA, posB, attrA, attrB)
    """
    pairs = []
    lookback_number = lookback[0]
    i = 0  # lower-bound: targets[0..i-1] all have pos <= last processed source pos

    for ea_ts, ea_pos, ea_attr in e_source:
        if last_checked is not None and ea_ts <= last_checked:
            continue

        # advance lower-bound past targets that cannot follow this source
        while i < len(e_target) and e_target[i][1] <= ea_pos:
            i += 1

        # pair with the first target after this source (don't advance i —
        # the target remains available for later sources with larger positions)
        if i < len(e_target):
...
def update_last_checked(previous_last_checked: DataFrame|None, current_last_checked: DataFrame, batch_min_ts: int, batch_min_pos: int, real_lookback: Tuple[int, LookbackType]) -> DataFrame:
    ...
    if real_lookback[1] == "time":
        ...
        return merged_last_checked_df.filter((col("last_checked_moment") >= batch_min_pos) | ((batch_min_pos - col("last_checked_moment")) < real_lookback[0]))